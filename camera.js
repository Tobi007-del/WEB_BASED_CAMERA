...
const DOM = {
    ...
    recorderVideoWrapper: container.querySelector(
    ...
    videoPickerWrapper: container.querySelector(
      ".video-section .section-picker-wrapper",
    ),
    videoPickerBtn: container.querySelector(
      ".video-section .media-picker-button",
    ),
    videoSettingsDropdown: container.querySelector(
      ".video-section .settings-choices-dropdown",
    ),
    audioPickerWrapper: container.querySelector(
      ".audio-section .section-picker-wrapper",
    ),
    audioPickerBtn: container.querySelector(
      ".audio-section .media-picker-button",
    ),
    audioSettingsDropdown: container.querySelector(
      ".audio-section .settings-choices-dropdown",
    ),
...
function deactivateStates({ target }) {
  Array.from(DOM.activeElements)
    .filter((element) => JSON.parse(element.dataset.active))
    .forEach((element) => {
...
async function handleMediaDeviceChange() {
  ...
  if (
    !userMediaDevices
      ...
      .find((device) => device.label === DOM.videoPickerBtn.textContent) ||
    !userMediaDevices
      .filter((device) => device.kind === "audioinput")
      .find((device) => device.label === DOM.audioPickerBtn.textContent)
  )
    startCapturingStream();
}
...
async function startCapturingStream() {
  try {
    ...
    const stream =
      await navigator.mediaDevices.getUserMedia(recorderConstraints);

    const videoTrack = stream.getVideoTracks()[0];
    const audioTrack = stream.getAudioTracks()[0];
    if (videoTrack) {
      const videoSettings = videoTrack.getSettings();
      DOM.videoPickerBtn.textContent = userMediaDevices
        .filter((device) => device.kind === "videoinput")
        .find((device) => device.deviceId === videoSettings.deviceId)?.label;
    }
    if (audioTrack) {
      const audioSettings = audioTrack.getSettings();
      DOM.audioPickerBtn.textContent = userMediaDevices
        .filter((device) => device.kind === "audioinput")
        .find((device) => device.deviceId === audioSettings.deviceId)?.label;
    } else DOM.audioPickerBtn.textContent = "Without audio";

    DOM.video.srcObject = stream;
    DOM.video.captureStream =
      DOM.video.captureStream || DOM.video.mozCaptureStream;
    DOM.video.addEventListener(
      "playing",
      () => (DOM.recorderSection.dataset.ready = true),
      { once: true },
    );
    DOM.video.play();
...
async function stall(delayInMs) {
  return new Promise((resolve) => {
    if (delayInMs > 1000) {
      DOM.timerTopWrapper.querySelector("p").textContent = Math.round(
        delayInMs / 1000,
      );
      DOM.timerBottomWrapper.querySelector("p").textContent = Math.round(
        delayInMs / 1000,
      );
      ...
      let stallInterval = setInterval(() => {
        ...
        if (activeTime < delayInMs) {
          const startNumber = parseInt(
            DOM.timerTopWrapper.querySelector("p").textContent,
          );
          const newNumber =
            Math.round(delayInMs / 1000) - Math.round(activeTime / 1000);
          ...
          DOM.topFlipWrapper.onanimationstart = () =>
            (DOM.timerTopWrapper.querySelector("p").textContent = newNumber);
          DOM.topFlipWrapper.onanimationend = () =>
            DOM.topFlipWrapper.classList.remove("flip");
          ...
          resolve(
            delayInMs / 1000 +
              " seconds elapsed, stalled user successfully... You can now ride on!",
          );
        }
      }, 1000);
...
function formatTime(time) {
  const hours =
    time >= 3600 ? formatter.format(Math.floor(time / 3600)) + ":" : "";
...
DOM.stallTimerBtns.forEach((btn) =>
  btn.addEventListener("click", editStallDelay),
);

DOM.closeSettingsBtn.addEventListener(
  "click",
  () => (DOM.contentFooterWrapper.dataset.active = "false"),
);

DOM.settingsBtn.addEventListener(
  "click",
  () =>
    (DOM.contentFooterWrapper.dataset.active = !JSON.parse(
      DOM.contentFooterWrapper.dataset.active,
    )),
);

DOM.videoPickerBtn.addEventListener(
  "click",
  () =>
    (DOM.videoPickerWrapper.dataset.active = !JSON.parse(
      DOM.videoPickerWrapper.dataset.active,
    )),
);

DOM.audioPickerBtn.addEventListener(
  "click",
  () =>
    (DOM.audioPickerWrapper.dataset.active = !JSON.parse(
      DOM.audioPickerWrapper.dataset.active,
    )),
);
